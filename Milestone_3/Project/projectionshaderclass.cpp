...
bool ProjectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Project/projection_vs.hlsl", L"../Project/projection_ps.hlsl");
	if(!result)
	{
		return false;
	}
...
bool ProjectionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
								   D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor,
								   D3DXVECTOR3 lightPosition, D3DXMATRIX viewMatrix2, D3DXMATRIX projectionMatrix2, 
								   ID3D11ShaderResourceView* projectionTexture, D3DXVECTOR3 lightDirection)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, texture2, ambientColor, diffuseColor, lightPosition, 
								 viewMatrix2, projectionMatrix2, projectionTexture, lightDirection);
	if(!result)
	{
		return false;
	}
...
bool ProjectionShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
												D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, D3DXVECTOR4 ambientColor,
