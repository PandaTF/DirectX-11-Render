...
bool SpecMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Project/specmap_vs.hlsl", L"../Project/specmap_ps.hlsl");
	if (!result)
	{
		return false;
	}
...
bool SpecMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	...
	float specularPower, D3DXVECTOR4 ambientColor)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureArray, lightDirection,
		diffuseColor, cameraPosition, specularColor, specularPower, ambientColor);
	if (!result)
	{
		return false;
	}
...
void SpecMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
